...
class OpencartAPI {
    ...
    public function getProducts($limit = 100, $page = 1) {
        try {
            ...
            // Ürün resimlerini tam URL'e çevir
            if (isset($result['products']) && is_array($result['products'])) {
                foreach ($result['products'] as &$product) {
                    // Ana resmi tam URL yap
                    if (!empty($product['image'])) {
                        $product['image'] = $this->makeFullImageUrl($product['image']);
                    }
                    
                    // Ek resimleri tam URL yap
                    if (!empty($product['images']) && is_array($product['images'])) {
                        foreach ($product['images'] as &$image) {
                            $image = $this->makeFullImageUrl($image);
                        }
                    }
                    
                    // Varyant resimlerini tam URL yap
                    if (!empty($product['variants']) && is_array($product['variants'])) {
                        foreach ($product['variants'] as &$variant) {
                            if (!empty($variant['image'])) {
                                $variant['image'] = $this->makeFullImageUrl($variant['image']);
                            }
                        }
                    }
                }
    ...
    /**
     * Relative image path'i tam URL'e çevir
     */
    private function makeFullImageUrl($imagePath) {
        if (empty($imagePath)) {
            return '';
        }
        ...
        // OpenCart genelde image/ ile başlayan path döndürür
        $imagePath = ltrim($imagePath, '/');
        return $this->storeUrl . '/image/' . $imagePath;
    }
