...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    elseif ($marketplace == 'hepsiburada') {
        $username = trim($_POST['hb_username'] ?? '');
        $password = trim($_POST['hb_password'] ?? '');
        ...
        try {
            ...
            if ($exists) {
                $stmt = $db->prepare("UPDATE marketplace_settings SET api_key = :username, api_secret = :password, merchant_id = :merchant_id, is_active = :is_active WHERE marketplace = 'hepsiburada'");
            } else {
                $stmt = $db->prepare("INSERT INTO marketplace_settings (marketplace, api_key, api_secret, merchant_id, is_active) VALUES ('hepsiburada', :username, :password, :merchant_id, :is_active)");
            }
    ...
    <div id="hepsiburada" class="tab-content">
        <div class="settings-card">
            ...
            <form method="POST" action="">
                ...
                <div class="form-row">
                    <label for="hb_username">Username</label>
                    <input type="text" id="hb_username" name="hb_username" 
                           value="<?php echo htmlspecialchars($settings['hepsiburada']['api_key'] ?? ''); ?>" required>
                    <small>Hepsiburada Merchant Panel'den alacağınız kullanıcı adı</small>
                </div>

                <div class="form-row">
                    <label for="hb_password">Password</label>
                    <input type="password" id="hb_password" name="hb_password" 
                           value="<?php echo htmlspecialchars($settings['hepsiburada']['api_secret'] ?? ''); ?>" required>
                    <small>Hepsiburada Merchant Panel'den alacağınız şifre</small>
                </div>

                <div class="form-row">
                    <label for="hb_merchant_id">Merchant ID</label>
                    <input type="text" id="hb_merchant_id" name="hb_merchant_id" 
                           value="<?php echo htmlspecialchars($settings['hepsiburada']['merchant_id'] ?? ''); ?>" required>
                    <small>Hepsiburada Merchant ID'niz</small>
                </div>
